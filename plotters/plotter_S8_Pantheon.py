...
kidsS8    = 0.745*np.sqrt(0.3)
kidssigma = 0.039*np.sqrt(0.3)
axs = axs.ravel()
...
for DE in range(len(DEtype)):
    ...
    for case in range(len(roots)):
       ...
       print 'Tension for case {} is {}'.format(roots[case]+'_'+DEtype[DE]+'_Planck_Pantheon_BAO.margestats', abs(kidsS8-mean)/np.sqrt(kidssigma**2.+sigma**2.))
       ...
       print 'Tension for case {} is {}'.format(roots[case]+'_'+DEtype[DE]+'_Planck_Pantheon.margestats', abs(kidsS8-mean)/np.sqrt(kidssigma**2.+sigma**2.))
...
plt.savefig('results_plots/errorbarS8_Pantheon.pdf', bbox_inches='tight')
plt.savefig('/data1/martinelli/Dropbox/Projects/SNsys/errorbarS8_Pantheon.pdf', bbox_inches='tight')
