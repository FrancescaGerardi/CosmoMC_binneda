...
def Eofz(z,wpars,om):
    ...
    if isinstance(wpars, (list, tuple, np.ndarray)):
       integrand = np.zeros(zsamp)
       for ind in range(zsamp):
       ...
       E = np.sqrt(om*(1+z)**3.+(1-om)*(1+z)**(3.*(1+wpars)))
...
for ind in range(len(plotred)):
        mu_base[ind] = 5*np.log(lumdist(plotred[ind],wconst,omegam,H0))
tlam2 = time()

print 'time for lumdist {} s'.format(tlam2-tlam2)
...
for case in range(len(bin_roots)):
    ...
    wconst   = [data.loc[data.parameter=='binw'+str(i+1),'mean'].values[0] for i in range(Nbins)] 
    omegam   = data.loc[data.parameter=='omegam*','mean'].values[0]
    H0       = data.loc[data.parameter=='H0*','mean'].values[0]
    #here sys pars
    ...
    for ind in range(len(plotred)):
        sys[ind] = getsys(plotred[ind],w_roots[case],data,np.array(wconst),omegam,H0)
...
for case in range(len(w_roots)):
    ...
    mu = np.empty(len(plotred))
    for ind in range(len(plotred)):
        mu[ind] = 5*np.log(lumdist(plotred[ind],wconst,omegam,H0))

    sys = np.empty(len(plotred))
    for ind in range(len(plotred)):
        sys[ind] = getsys(plotred[ind],w_roots[case],data,wconst,omegam,H0)
...
for case in range(len(lam_roots)):
    ...
    sys = np.empty(len(plotred))
    for ind in range(len(plotred)):
        sys[ind] = getsys(plotred[ind],lam_roots[case],data,wconst,omegam,H0)
