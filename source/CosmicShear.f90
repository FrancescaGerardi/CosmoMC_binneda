...
!    logical :: use_morellz = .true. !if true use 101 ell values, otherwise 31 ell values
    ...
    nangbins = 9
    nzbins = 4
    if(.not. use_morellz) nellbins = 31
    if(use_morellz) nellbins = 101
    ...
    if(.not. use_morellz) morellfac = 0.5
    if(use_morellz) morellfac = 0.1
    ...
    if(use_rombintz) then !Romberg Integration for the outer integral
        ...
        do bin = 1,nzbins*(nzbins+1)/2
            ...
            !$OMP PARALLEL DO PRIVATE(ellgen,obj)
                do ellgen=1,nellbins
    ...
    if(.not. use_rombintz) then
        ...
        do binw = 1,nzbins
            ...
            !$OMP PARALLEL DO PRIVATE(wttt,obj)
        ...
        do bin = 1,nzbins*(nzbins+1)/2
            ...
            do ellgen=1,obj%nellbinsomp
                ...
                !$OMP PARALLEL DO PRIVATE(ttt,obj)
                    do ttt=2,36
    ...
    do biin = 1,nzbins*(nzbins+1)/2
        ...
        !$OMP PARALLEL DO PRIVATE(obj,ellgen)
            do ellgen=1,58999
